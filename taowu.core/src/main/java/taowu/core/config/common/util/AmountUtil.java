...
public abstract class AmountUtil {
	 ...
	 */
	public static boolean toEqual(BigDecimal a, BigDecimal b,int scale) {
		BigDecimal v1 = a.setScale(scale,BigDecimal.ROUND_HALF_UP);
		BigDecimal v2 = b.setScale(scale,BigDecimal.ROUND_HALF_UP);
	 ...
	 * @return a==b 返回true, a!=b 返回false
	 */
	public static boolean toEqual(double a, double b,int scale) {
		BigDecimal v1 = BigDecimal.valueOf(a).setScale(scale,BigDecimal.ROUND_HALF_UP);
		BigDecimal v2 = BigDecimal.valueOf(b).setScale(scale,BigDecimal.ROUND_HALF_UP);
		if (v1.compareTo(v2) == 0) {
			return true;
		}
		return false;
